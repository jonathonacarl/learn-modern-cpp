...
public:
    ...
    {
        const size_t head = m_head.load(std::memory_order_relaxed); // Producer calls this, so it's the only one looking.
        const size_t tail = m_tail.load(std::memory_order_acquire); // Producer needs to acquire what the consumer has published.
    ...
    {
        const size_t head = m_head.load(std::memory_order_acquire); // Consumer needs to acquire what the producer has published.
        const size_t tail = m_tail.load(std::memory_order_relaxed); // Consumer calls this, so it's the only one looking.
